...
class AudioTransform():
    ...
    def __getitem__(self,idx):
        ...
        print(reaud)
        print('RE sample shape: ',reaud[0].shape)
        rechan=AudioUtil.rechannel(reaud,self.channel)
        print('RE channel shape: ',rechan[0].shape)
        dur_aud=AudioUtil.pad_trunc(rechan,self.duration)
        print('pad shape: ',dur_aud[0].shape)
        shift_aud=AudioUtil.time_shift(dur_aud,self.shift_pct)
        print('shift shape: ',shift_aud[0][0].shape)
        print(shift_aud[0][0].shape)
        print(shift_aud[0][1].shape)
        sgram=AudioUtil.spectro_gram(shift_aud,n_mels=64,n_fft=1024,hop_len=None)
        print('spectogram shape: ',sgram.shape)
